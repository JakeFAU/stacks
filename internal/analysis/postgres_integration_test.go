...
func TestCanonicalManagerQueryPostgresCancellation(t *testing.T) {
	...
	database, err := postgres.Open(context.Background(), isolated.ApplicationURL())
	if err != nil {
	...
	_, err = repository.LoadPairInputs(
		ctx,
		"employee:opaque/canceled",
		"manager:opaque/canceled",
	)
	if !errors.Is(err, context.Canceled) {
		t.Fatalf("LoadPairInputs() error = %v, want context.Canceled", err)
	}
}
